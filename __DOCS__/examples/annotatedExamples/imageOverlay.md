...
myImage.overlay(
  Overlay.source(
    // An overlay has a Source and a Position Qualifiers
    image("cloudinary_icon") // Mandatory - Source
      .transformation(
        new Transformation().resize(scale().width(300).height(300)) // A source can be transformed like an image
      )
  ).position(new Position().gravity(compass(southEast())))
);
myImage.toString();
// l_cloudinary_icon/c_scale,h_300,w_300/fl_layer_apply,g_south_east
```
