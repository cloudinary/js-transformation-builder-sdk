  ...
  if (typeof linkText !== "string") {
    ...
    if (found.length && found[0].kind) {
      ...
      if (doclet.kind === "tutorial") {
        return helper.toTutorial(doclet.longname, text, {
          tag: "em",
          classname: "disabled",
        });
      }
  ...
  Object.keys(pages).forEach(function (kind) {
    if (pages[kind].length) {
      pages[kind].forEach(function (doclet) {
        var filename =
          doclet.kind === "tutorial" ? helper.tutorialToUrl(doclet.longname) : helper.longnameToUrl[doclet.longname];
        if (!filename) return;
...
var hasNavMember = (exports.hasNavMember = function (kind) {
  return (
    options.navMembers.findIndex(function (member) {
