(function (options, enabled, $) {
  ...
  $.extend(AccessFilter.prototype, {
    init: function () {
      var self = this;
      self.$toggleInherited = $(".access-filter .toggle-inherited").on(
        "change",
        { self: self },
        self.onInheritedChanged
      );
      self.setInherited();
      self.$togglePublic = $(".access-filter .toggle-public").on("change", { self: self }, self.onPublicChanged);
      self.setPublic();
      self.$toggleProtected = $(".access-filter .toggle-protected").on(
        "change",
        { self: self },
        self.onProtectedChanged
      );
      self.setProtected();
