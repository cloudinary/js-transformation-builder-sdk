...
var supportsParams = function (doclet) {
  return (
    ...
    (doclet.kind === "typedef" &&
      !!doclet.type &&
      !!doclet.type.names &&
      doclet.type.names.some(function (name) {
...
exports.getAttribs = function (doclet) {
  if (supportsParams(doclet) || doclet.kind === "member" || doclet.kind === "constant") {
    ...
    return attribs.length
      ? '<span class="signature-attribs">' + helper.htmlsafe("<" + attribs.join(", ") + "> ") + "</span>"
      : "";
  }
  return "";
};
...
exports.getParamsOrProps = function (doclet, type) {
  ...
  doclet[type].forEach(function (paramOrProp) {
    ...
    var parts = paramOrProp.name.split("."),
      last = parts.length - 1,
      base = sorted,
      parentName = [];
    parts.forEach(function (part, i) {
      ...
      if (i === last) {
        ...
        (index = base[type].findIndex(function (p) {
          return p.name === part;
        })) !== -1
      ) {
...
exports.getPageTitle = function (doclet, sanitized) {
  ...
  if (
    template.kinds.pages.indexOf(doclet.kind) !== -1 &&
    template.kinds.custom.indexOf(doclet.kind) === -1 &&
    doclet.ancestors &&
    doclet.ancestors.length
  ) {
  ...
  if (doclet.title) {
    ...
    if (doclet.exported) {
      name =
        name.replace(
          "module:",
          '<span class="name-signature">(<span class="name-require">require</span>(<span class="name-string">"'
        ) + '"</span>))</span>';
    }
...
exports.getShowAccessFilter = function (doclet) {
  var result =
    typeof doclet.showAccessFilter != "boolean" ? template.options.showAccessFilter : doclet.showAccessFilter;
  if (result) {
    ...
    var count =
      (doclet.has.inherited ? 1 : 0) +
      (doclet.has.public ? 1 : 0) +
      (doclet.has.protected ? 1 : 0) +
      (doclet.has.private ? 1 : 0);
