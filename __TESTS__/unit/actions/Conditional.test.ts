...
describe("Tests for Transformation Action -- Conditional", () => {
  it("Creates a conditional transformation", () => {
    const tx = new Transformation()
      .conditional(Conditional.ifCondition("ar >= 1.0", new Transformation().addAction("w_100")))
      .toString();
  ...
  it("Conditional with if/else combination", () => {
    const tx = new Transformation()
      .conditional(
        Conditional.ifCondition("ar >= 1.0", new Transformation().resize(Resize.scale(50))).otherwise(
          new Transformation().resize(Resize.scale(200))
        )
      )
      .toString();
