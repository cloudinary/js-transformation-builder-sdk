...
describe("Tests for Transformation Action -- Transcode", () => {
  it("Creates a Transformation with audiocodec", () => {
    const tx = new Transformation().transcode(Transcode.audioCodec(AudioCodec.aac())).toString();

    expect(tx).toBe("ac_aac");
  });

  it("Can use shortened audioCodec notation", () => {
    const tx = new Transformation().transcode(audioCodec("aac")).toString();

    expect(tx).toBe("ac_aac");
  });

  it("Creates a Transformation with bitrate", () => {
    const tx = new Transformation().transcode(Transcode.bitRate("500k")).toString();

    expect(tx).toBe("br_500k");
  });

  it("Creates a Transformation with bitrate.constant", () => {
    const tx = new Transformation().transcode(Transcode.bitRate(500).constant()).toString();

    expect(tx).toBe("br_500:constant");
  });

  it("Creates a Transformation with audioFrequency", () => {
    const tx = new Transformation().transcode(Transcode.audioFrequency(AudioFrequency.FREQ11025())).toString();

    expect(tx).toBe("af_11025");
  });

  it("Can use shortened audioFrequency notation", () => {
    const tx = new Transformation().transcode(Transcode.audioFrequency(11025)).toString();

    expect(tx).toBe("af_11025");
  });

  it("Creates a Transformation with fps", () => {
    const tx = new Transformation().transcode(Transcode.fps(20)).toString();

    expect(tx).toBe("fps_20");
  });

  it("Creates a Transformation with fpsRange from", () => {
    const tx = new Transformation().transcode(Transcode.fpsRange(20)).toString();

    expect(tx).toBe("fps_20-");
  });

  it("Creates a Transformation with fpsRange from-to", () => {
    const tx = new Transformation().transcode(Transcode.fpsRange(20, 20)).toString();
  ...
  it("Creates a Transformation with keyframeInterval", () => {
    const tx = new Transformation()
      .transcode(Transcode.keyframeInterval(0.4))
      .transcode(Transcode.keyframeInterval("0.8"))
      .toString();
  ...
  it("Creates a Transformation with streamingProfile", () => {
    const tx = new Transformation().transcode(Transcode.streamingProfile(StreamingProfile.fullHd())).toString();

    expect(tx).toBe("sp_full_hd");
  });

  it("Can use shortened streamingProfile notation", () => {
    const tx = new Transformation().transcode(Transcode.streamingProfile("full_hd")).toString();

    expect(tx).toBe("sp_full_hd");
  });

  it("Creates a Transformation with toAnimated", () => {
    const tx = new Transformation().transcode(Transcode.toAnimated(AnimatedFormat.gif())).toString();

    expect(tx).toBe("f_gif,fl_animated");
  });

  it("Creates a Transformation with toAnimated webp", () => {
    const tx = new Transformation().transcode(Transcode.toAnimated(AnimatedFormat.webp())).toString();

    expect(tx).toBe("f_webp,fl_animated,fl_awebp");
  });

  it("Can use shortened toAnimated webp", () => {
    const tx = new Transformation().transcode(Transcode.toAnimated("webp")).toString();

    expect(tx).toBe("f_webp,fl_animated,fl_awebp");
  });

  it("Creates a Transformation with toAnimated and delay", () => {
    const tx = new Transformation().transcode(Transcode.toAnimated("gif").delay(20)).toString();
  ...
  it("Creates a Transformation with toAnimated, delay, sampling", () => {
    const tx = new Transformation()
      .transcode(Transcode.toAnimated(AnimatedFormat.gif()).delay(20).sampling("4s"))
      .toString();
  ...
  it("Tests for simple videoCodec", () => {
    const tx = new Transformation().transcode(Transcode.videoCodec(VideoCodec.vp9())).toString();
  ...
  it("Tests for adv videoCodec", () => {
    const tx = new Transformation()
      ...
      .transcode(
        Transcode.videoCodec(VideoCodec.h264().profile(VideoCodecProfile.baseline()).level(VideoCodecLevel.vcl31()))
      )
      .toString();
