...
describe("Tests for overlay actions", () => {
  it("Tests Image on Image with publicID encoding", () => {
    ...
    tx.overlay(Overlay.source(Source.image("path/to/sample").format(Format.png())));
  ...
  it("Tests Image on Image with format", () => {
    const tx = new Transformation();
    tx.overlay(Overlay.source(Source.image("sample").format(Format.png())));
  ...
  it("Tests Image on Image with BlendMode and Position", () => {
    ...
    tx.overlay(Overlay.source(Source.image("sample")).position(sampleFacePosition()).blendMode(BlendMode.screen()));
  ...
  it("Can use shortened BlendMode notation", () => {
    ...
    tx.overlay(Overlay.source(Source.image("sample")).position(sampleFacePosition()).blendMode("mask"));
  ...
  it("Tests text on image", () => {
    ...
    tx.overlay(Overlay.source(Source.text("Testing", textStyle).backgroundColor("red").textColor("blue")));
  ...
  it("Tests text on image with RGB backgroundColor", () => {
    ...
    tx.overlay(
      Overlay.source(Source.text("Testing", new TextStyle("arial", 15)).backgroundColor("#FEB61FC2").textColor("blue"))
    );
  ...
  it("Tests nested subtitles on image", () => {
    ...
    tx.overlay(Overlay.source(Source.subtitles("path/to/subs.srt").textStyle(textStyle)));
  ...
  it("Tests subtitle on without style", () => {
    ...
    tx.overlay(Overlay.source(Source.subtitles("subs.srt")));
  ...
  it("Tests subtitle on image", () => {
    ...
    tx.overlay(Overlay.source(Source.subtitles("subs.srt").textStyle(textStyle)));
  ...
  it("Tests an overlay with a complete example", () => {
    ...
    tx.overlay(
      Overlay.source(Source.image("sample").transformation(sampleTxResizePad()))
  ...
  it("Tests an image underlay with a complete example", () => {
    ...
    tx.underlay(
      Underlay.source(Source.image("sample").transformation(sampleTxResizePad()))
        .position(sampleFacePosition())
        .blendMode(BlendMode.screen())
    );
  ...
  it("Video on Video with timeline", () => {
    ...
    tx.overlay(
      Overlay.source(Source.video("sample").transformation(sampleTxResizePad()))
  ...
  it("Tests a fetchSource with format", () => {
    ...
    tx.overlay(Overlay.source(Source.fetch(REMOTE_URL).transformation(sampleTxResizePad()).format(png())));

    // This is a fully functional URL that should work in the browser.
    // Explicitly check the resulting base64 string
    expect(tx.toString()).toContain(
      `l_fetch:aHR0cHM6Ly9yZXMuY2xvdWRpbmFyeS5jb20vZGVtby9pbWFnZS91cGxvYWQvY2k=.png/${sampleTxResizePad().toString()}`
    );
  });
  ...
  it("should serialize a unicode url of fetch source", () => {
    ...
    const expected =
      "l_fetch:aHR0cHM6Ly91cGxvYWQud2lraW1lZGlhLm9yZy93aWtpcGVkaWEvY29tbW9ucy8yLzJiLyVFQSVCMyVBMCVFQyVCMCVCRCVFQSVCMCVBRiVFQiVCMiU4Qy5qcGc=/fl_layer_apply";
    const actual = tx.overlay(Overlay.source(Source.fetch(REMOTE_URL))).toString();
