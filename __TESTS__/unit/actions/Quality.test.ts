...
describe("Tests for Transformation Action -- Delivery.quality", () => {
  ...
  it("Creates a Transformation with quality:best", () => {
    const tx = new Transformation().delivery(Delivery.quality(Quality.autoBest())).toString();

    expect(tx).toBe("q_auto:best");
  });

  it("Creates a Transformation with quality:eco", () => {
    const tx = new Transformation().delivery(Delivery.quality(Quality.autoEco())).toString();

    expect(tx).toBe("q_auto:eco");
  });

  it("Creates a Transformation with quality:good", () => {
    const tx = new Transformation().delivery(Delivery.quality(Quality.autoGood())).toString();

    expect(tx).toBe("q_auto:good");
  });

  it("Creates a Transformation with quality:low", () => {
    const tx = new Transformation().delivery(Delivery.quality(Quality.autoLow())).toString();

    expect(tx).toBe("q_auto:low");
  });

  it("Creates a Transformation with jpegmini", () => {
    const tx = new Transformation().delivery(Delivery.quality(Quality.jpegmini())).toString();

    expect(tx).toBe("q_jpegmini");
  });

  it("Creates a Transformation with jpegmini HIGH", () => {
    const tx = new Transformation().delivery(Delivery.quality(Quality.jpegminiHigh())).toString();

    expect(tx).toBe("q_jpegmini:1");
  });

  it("Creates a Transformation with jpegmini MEDIUM", () => {
    const tx = new Transformation().delivery(Delivery.quality(Quality.jpegminiMedium())).toString();

    expect(tx).toBe("q_jpegmini:2");
  });

  it("Creates a Transformation with jpegmini BEST", () => {
    const tx = new Transformation().delivery(Delivery.quality(Quality.jpegminiBest())).toString();
  ...
  it("Sets Quanitzation level", () => {
    const tx = new Transformation().delivery(Delivery.quality("75").quantization(123)).toString();
