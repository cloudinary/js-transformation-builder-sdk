...
describe("Tests for Transformation Action -- Effect", () => {
  ...
  it("Creates a Transformation with effect colorize", () => {
    const tx = new Transformation().effect(Effect.colorize(10).color("red")).toString();

    expect(tx).toBe("co_red,e_colorize:10");
  });

  it("Creates a Transformation with effect colorize:level", () => {
    const tx = new Transformation().effect(Effect.colorize(50)).toString();

    expect(tx).toBe("e_colorize:50");
  });

  it("Creates a Transformation with effect colorize:level", () => {
    const tx = new Transformation().effect(Effect.colorize().level(10)).toString();

    expect(tx).toBe("e_colorize:10");
  });

  it("Creates a Transformation with effect oilPaint", () => {
    const tx = new Transformation().effect(Effect.oilPaint().strength(10)).toString();

    expect(tx).toBe("e_oil_paint:10");
  });

  it("Creates a Transformation with effect oilPaint:level", () => {
    const tx = new Transformation().effect(Effect.oilPaint(50)).toString();

    expect(tx).toBe("e_oil_paint:50");
  });

  it("Creates a Transformation with effect artisticFilter", () => {
    const tx = new Transformation().effect(Effect.artisticFilter(ArtisticFilter.peacock())).toString();

    expect(tx).toBe("e_art:peacock");
  });

  it("Can use shortened artisticFilter notation", () => {
    const tx = new Transformation().effect(Effect.artisticFilter("al_dente")).toString();

    expect(tx).toBe("e_art:al_dente");
  });

  it("Creates a Transformation with effect cartoonify:50", () => {
    const tx = new Transformation().effect(cartoonify().lineStrength(50).blackwhite()).toString();
  ...
  it("Creates a Transformation with effect style_transfer", () => {
    const tx = new Transformation().effect(Effect.styleTransfer(image("woman"))).toString();
  ...
  it("Creates a Transformation with effect style_transfer:strength", () => {
    const tx = new Transformation().effect(Effect.styleTransfer(image("woman")).strength(15)).toString();
  ...
  it("Creates a Transformation with effect style_transfer:preserve_color:strength", () => {
    const tx = new Transformation()
      .effect(Effect.styleTransfer(image("woman")).strength(15).preserveColor())
      .toString();
  ...
  it("Creates a Transformation with effect style_transfer:preserve_color:strength with a transformation", () => {
    const tx = new Transformation()
      .effect(
        Effect.styleTransfer(image("woman").transformation(new Transformation().resize(scale(100, 100))))
          .strength(15)
          .preserveColor()
      )
      .toString();
  ...
  it("Test Vectorize", () => {
    expect(Effect.vectorize().toString()).toEqual("e_vectorize");
  ...
  it("Test gradientFade", () => {
    ...
    expect(Effect.gradientFade().toString()).toBe("e_gradient_fade");

    expect(Effect.gradientFade().strength(5).toString()).toBe("e_gradient_fade:5");
  ...
  it("Test assistColorBlind", () => {
    expect(Effect.assistColorBlind().toString()).toBe("e_assist_colorblind");

    expect(Effect.assistColorBlind().xray().toString()).toBe("e_assist_colorblind:xray");

    expect(Effect.assistColorBlind().xray().stripesStrength(10).toString()).toBe("e_assist_colorblind:10");

    expect(Effect.assistColorBlind().stripesStrength(10).xray().toString()).toBe("e_assist_colorblind:xray");
  });

  it("Test Effect.outline", () => {
    // co_{color},e_outline:{mode}:{width}:{blurLevel}
    expect(Effect.outline().toString()).toBe("e_outline");

    expect(Effect.outline().mode(OutlineMode.fill()).width(10).blurLevel(25).color(Color.BLUE).toString()).toBe(
      "co_blue,e_outline:fill:10:25"
    );
  });

  it("Can use shortened Effect.outline notation", () => {
    expect(Effect.outline().mode("fill").width(10).blurLevel(25).color(Color.BLUE).toString()).toBe(
      "co_blue,e_outline:fill:10:25"
    );
  });
  ...
  it("Test simulateColorBlind", () => {
    ...
    expect(Effect.simulateColorBlind().condition(rodMonochromacy()).toString()).toBe(
      "e_simulate_colorblind:rod_monochromacy"
    );
  });

  it("Can shorten simulateColorBlindType notation", () => {
    expect(Effect.simulateColorBlind().condition("deuteranomaly").toString()).toBe(
      "e_simulate_colorblind:deuteranomaly"
    );
  });
