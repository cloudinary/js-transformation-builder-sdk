...
describe("Tests for Transformation Action -- VideoEdit", () => {
  it("Creates a Transformation with concatenate", () => {
    const tx = new Transformation().videoEdit(VideoEdit.concatenate(Concatenate.videoSource("dog"))).toString();
  ...
  it("Creates a Transformation with concatenate prepend", () => {
    const tx = new Transformation()
      .videoEdit(
        VideoEdit.concatenate(Concatenate.videoSource("dog")).prepend() // implicitly true
      )
      .toString();
  ...
  it("Creates a Transformation with concatenate with transition", () => {
    ...
    const tx = new Transformation()
      .videoEdit(VideoEdit.concatenate(src).transition(Transition.videoSource("transition1")).duration(5))
      .toString();
  ...
  it("Creates a Transformation with trim", () => {
    const tx = new Transformation().videoEdit(VideoEdit.trim().startOffset(3).endOffset(4).duration(10)).toString();
  ...
  it("Creates a Transformation with trim %", () => {
    const tx = new Transformation()
      .videoEdit(VideoEdit.trim().startOffset("3%").endOffset("4%").duration("10%"))
      .toString();
  ...
  it("Creates a Transformation with volume string", () => {
    const tx = new Transformation().videoEdit(VideoEdit.volume("5db")).toString();

    expect(tx).toBe("e_volume:5db");
  });

  it("Creates a Transformation with volume number", () => {
    const tx = new Transformation().videoEdit(VideoEdit.volume(10)).toString();

    expect(tx).toBe("e_volume:10");
  });

  it("Creates a Transformation with volume Volume.mute()", () => {
    const tx = new Transformation().videoEdit(VideoEdit.volume(Volume.mute())).toString();

    expect(tx).toBe("e_volume:mute");
  });

  it("Creates a Transformation with volume Volume.byDecibels()", () => {
    const tx = new Transformation().videoEdit(VideoEdit.volume(Volume.byDecibels(5))).toString();

    expect(tx).toBe("e_volume:5db");
  });

  it("Creates a Transformation with volume Volume.byPercent()", () => {
    const tx = new Transformation().videoEdit(VideoEdit.volume(Volume.byPercent("10"))).toString();
  ...
  it("Tests a preview transformation for a video", () => {
    const tx = new Transformation()
      .videoEdit(VideoEdit.preview().duration(5).minimumSegmentDuration(1).maximumSegments(10))
      .toString();
