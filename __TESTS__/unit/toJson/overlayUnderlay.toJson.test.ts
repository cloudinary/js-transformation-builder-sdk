...
describe("Overlay & Underlay toJson", () => {
  it("Should generate Overlay model for image source", () => {
    ...
    transformation.addAction(
      Overlay.source(Source.image("sample").transformation(new Transformation().resize(scale(100).aspectRatio(7))))
        .position(
          new Position().offsetX(1).offsetY(2).tiled().allowOverflow().gravity(Gravity.compass(Compass.northEast()))
        )
          ...
          source: {
            ...
            transformation: {
              actions: [
                {
                  actionType: "scale",
                  dimensions: { width: 100, aspectRatio: "7.0" },
                },
              ],
            },
          },
  ...
  it("Should generate Overlay model for fetch source", () => {
    ...
    transformation.addAction(
      Overlay.source(
        Source.fetch("https://some/image.jpg").transformation(new Transformation().resize(scale(100).aspectRatio(7)))
      )
        .position(
          new Position().offsetX(1).offsetY(2).tiled().allowOverflow().gravity(Gravity.compass(Compass.northEast()))
        )
  ...
  it("Should return error when there is unsupported transformation inside", () => {
    ...
    transformation.addAction(
      Overlay.source(
        Source.image("sample").transformation(new Transformation().resize(scale(100).width(800)).backgroundColor("red"))
      )
    );
