...
describe("Gravity Qualifier", () => {
  ...
  it("AutoGravity with focus should return g_auto:{obj}:{obj}", () => {
    const gAutoStr = Gravity.autoGravity()
      .autoFocus(AutoFocus.focusOn(FocusOn.person()), AutoFocus.focusOn(FocusOn.cat()))
      .toString();
  ...
  it("AutoGravity with weight and avoid", () => {
    const gAutoStr = Gravity.autoGravity()
      .autoFocus(AutoFocus.focusOn(FocusOn.person()).weight(100), AutoFocus.focusOn(FocusOn.cat()).avoid())
      .toString();
