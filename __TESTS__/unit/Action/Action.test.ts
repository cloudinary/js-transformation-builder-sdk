...
describe("Tests for Transformation Action", () => {
  it("Creates a transformation with new action while passing a qualifier to addQualifier()", () => {
    ...
    const tx = new Transformation().addAction(action).toString();
  ...
  it("Creates a transformation with new action using addValue()", () => {
    const action = new Action().addQualifier(
      new Qualifier("a", new QualifierValue([90, 80]).setDelimiter(".")).addValue(70)
    );
  ...
  it("Creates a transformation with new action while calling addQualifier twice", () => {
    const action = new Action().addQualifier(new Qualifier("w", 100)).addQualifier(new Qualifier("c", "fill"));
  ...
  it("Creates a transformation with new action, overwriting existing qualifier", () => {
    const action = new Action().addQualifier(new Qualifier("w", 100)).addQualifier(new Qualifier("w", 200));
  ...
  it("Creates a transformation with new action while adding a single flag", () => {
    const action = new Action().addQualifier(new Qualifier("l", "sample")).addFlag(new FlagQualifier("layer_apply"));
  ...
  it("Add and read actions tags", () => {
    const action = new Action().setActionTag("testing");
